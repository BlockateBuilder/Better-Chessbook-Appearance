(() => {
  ...
  let settings = load();
  let boardObserver;
  // This cache is populated only from images served by chessbook.com itself.
  // It deliberately records the complete native path, including the site's asset
  // set, instead of assuming that a visually similar public piece set is native.
  const chessbookSources = new Map();
  ...
  function nativeChessbookPiece(src) {
    // A Chessbook icon is verified only when the page itself serves it from its
    // /pieces/ directory and the filename agrees with the encoded piece identity.
    // In particular, never classify lichess1.org's cburnett files as Chessbook.
    try {
      const url = new URL(src, location.href);
      if (url.origin !== location.origin) return null;
      const match = url.pathname.match(/^\/pieces\/[^/]+\/(w|b)([KQRBNP])\.svg$/i);
      if (!match) return null;
      return { piece: match[1].toLowerCase() + match[2].toUpperCase(), source: url.pathname + url.search };
    } catch (_) { return null; }
  }
  function rememberChessbookSource(src) {
    const verified = nativeChessbookPiece(src);
    if (verified) chessbookSources.set(verified.piece, verified.source);
    return verified;
  }
  function chessbookSource(piece, savedSource = '') {
    // Restore only an asset Chessbook served during this session (or the verified
    // saved native URL). There is intentionally no /pieces/cburnett fallback:
    // that name is also used by Lichess and cannot prove the artwork is Chessbook's.
    const saved = nativeChessbookPiece(savedSource);
    if (saved?.piece === piece) {
      chessbookSources.set(piece, saved.source);
      return saved.source;
    }
    return chessbookSources.get(piece) || null;
  }
  function pieceSource(piece, original) {
    switch (settings.style) {
      case 'chessbook': return chessbookSource(piece, original);
      ...
      default: return chessbookSource(piece, original);
    }
  }
  function applyPieces(root = document) {
    root.querySelectorAll('[data-testid="chessboard"] #pieces-layer img').forEach(img => {
      ...
      const rendered = rememberChessbookSource(source);
      // Chessbook can reuse an image node when showing a course move or promotion.
      // A freshly rendered first-party source is the only time its piece identity
      // and restore target may be refreshed.
      if (rendered) {
        img.dataset.twPiece = rendered.piece;
        img.dataset.twOriginalSrc = rendered.source;
      }
      const piece = img.dataset.twPiece;
      if (!piece) return;
      const original = img.dataset.twOriginalSrc || '';
      const desired = pieceSource(piece, original);
      // Do not overwrite an unverified image with a guessed asset. Once Chessbook
      // renders the piece, its exact first-party source is cached and restored.
      if (desired && img.getAttribute('src') !== desired) img.setAttribute('src', desired);
      img.onerror = () => {
        if (settings.style !== 'chessbook' && original) img.src = original;
      };
    });
  }
  ...
  function observeBoard() {
    ...
    // Observe the board rather than only #pieces-layer: course navigation can
    // replace that layer entirely. Watching src catches a reused image immediately
    // when Chessbook changes it during a move or promotion.
    const observer = new MutationObserver(records => {
      if (records.some(record => record.type === 'childList' || (record.type === 'attributes' && record.attributeName === 'src'))) {
        applyAppearance();
      }
    });
